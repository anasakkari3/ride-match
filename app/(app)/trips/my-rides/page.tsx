            ...
            {myBookings.length === 0 && myTrips.length === 0 ? (
                <div className="rounded-3xl border border-slate-200 dark:border-slate-800 bg-slate-50 dark:bg-slate-900/50 p-8 text-center shadow-sm">
                    ...
                    <div className="flex flex-col gap-3">
                        <Link href="/app" className="w-full rounded-xl bg-sky-600 dark:bg-sky-500 hover:bg-sky-700 dark:hover:bg-sky-600 text-white px-6 py-3.5 text-sm font-bold shadow-sm transition-colors btn-press">
                            {t('find_a_ride')}
                        </Link>
                        <Link href="/trips/new" className="w-full rounded-xl bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 px-6 py-3.5 text-sm font-bold text-slate-700 dark:text-slate-300 shadow-sm transition-colors btn-press hover:bg-slate-50 dark:hover:bg-slate-700">
                            {t('create_a_ride')}
                        </Link>
                    </div>
                </div>
                ...
                <div className="space-y-4">
                    ...
                    {myTrips.length > 0 && (
                        <section className="space-y-3 pt-4">
                            <h2 className="text-xs font-bold text-slate-400 uppercase tracking-widest px-1">{t('driver')}</h2>
                            <div className="space-y-3">
                                {myTrips.map((trip) => (
                                    <Link key={trip.id} href={`/trips/${trip.id}`} className="block rounded-3xl border border-slate-100 dark:border-slate-800 bg-white dark:bg-slate-900 p-4 shadow-sm card-hover relative overflow-hidden group">
                                        {(() => {
